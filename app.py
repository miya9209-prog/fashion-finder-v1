...
DEFAULTS={'page':'home','query':'','selected_product_id':None,'selected_category':'상의','selected_middle':'전체','selected_brand':None,'banner_index':0}
for k,v in DEFAULTS.items():
    st.session_state.setdefault(k,v)

BANNERS=[
 {'eyebrow':"TODAY'S FIND",'title':'지금부터 가장 많이 입을\n간절기 출근룩','sub':'77까지 · 체형커버 · 오늘출발','cta':'출근룩 10,276개 보러가기','query':'77까지 체형커버 출근룩 오늘출발','image':'assets/hero/main_01.jpg'},
 {'eyebrow':'4050 BEST','title':'편한데 단정한\n팬츠를 찾고 있다면','sub':'허리밴딩 · 하체커버 · 77까지','cta':'팬츠 BEST 8,412개 보러가기','query':'77까지 하체커버 팬츠','image':'assets/hero/main_02.jpg'},
 {'eyebrow':'SAME DAY','title':'오늘 주문하면\n바로 출발하는 옷','sub':'당일발송 가능 상품만 모아보기','cta':'오늘출발 28,796개 보러가기','query':'오늘출발','image':'assets/hero/main_03.jpg'}]
QUICK=[('출근룩 최근 80%','출근룩','assets/quick/q01.jpg'),('연휴 여행룩','여행룩','assets/quick/q02.jpg'),('간절기 가디건','가디건','assets/quick/q03.jpg'),('체형커버 슬랙스','체형커버 슬랙스','assets/quick/q04.jpg'),('신경쓰이는 모임룩','모임룩','assets/quick/q05.jpg')]
...
def nav_header():
    with st.container(key='topnav'):
        ...
        with nav:
            ...
            for col,(label,page,val) in zip(cols,items):
                with col:
        ...
        with util:
            a,b=st.columns(2)
            with a: st.markdown("<div class='nav-icon'>♡</div>",unsafe_allow_html=True)
            with b: st.markdown("<div class='nav-icon bag'>BAG</div>",unsafe_allow_html=True)
...
def live_board():
    ...
    st.markdown(f"""<div class='live-board'><div class='live-head'><span><span class='live-dot'></span>FINDER LIVE</span><span class='live-time'>{now}</span></div><div class='live-body'><div class='live-kicker'>당일발송 가능한 상품 대기중</div><div class='live-main'><span class='live-prefix'>현재</span><span class='live-number'>{LIVE_PRODUCT_COUNT:,}</span><span class='live-unit'>개</span></div><div class='live-brand'>{LIVE_BRAND_COUNT}개 브랜드</div></div></div>""",unsafe_allow_html=True)
...
def search_page():
    ...
    chips=''.join([f"<span class='filter-chip'>{x}</span>" for x in labels])
    ...
    with pcol:
        ...
        rdf=rdf.sort_values('price' if sort!='추천순' else 'rank_score',ascending=(sort!='높은가격순' and sort!='추천순') if sort!='추천순' else False)
        render_grid(rdf,'search',3,12)

def category_page():
    cat=st.session_state.selected_category
    st.markdown(f"<div class='eyebrow'>CATEGORY</div><h2 class='page-title'>{cat}</h2>",unsafe_allow_html=True)
...
def brand_detail_page():
    ...
    st.markdown(f"<div class='eyebrow'>BRAND</div><h2 class='page-title'>{brand}</h2><div class='brand-desc'>{info.description}</div>",unsafe_allow_html=True)
...
def product_page():
    ...
    with b:
        ...
        st.markdown("<div class='match-label'>FINDER MATCH</div><div class='chip-row'>"+''.join([f"<span class='filter-chip'>{x}</span>" for x in chips])+"</div>",unsafe_allow_html=True)
