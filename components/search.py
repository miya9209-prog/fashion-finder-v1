...
TPO={'출근':'출근','회사':'출근','오피스':'출근','모임':'모임','하객':'하객','여행':'여행','휴가':'여행','주말':'주말','데일리':'데일리','시댁':'모임'}
...
def parse_query(query,brands):
 ...
 for brand in brands:
  if str(brand).lower() in q.lower(): p['brand']=brand; break
 ...
 if any(w in q for w in ['오늘출발','오늘 출발','당일발송','당일 발송','바로배송','바로 배송']):p['same_day']=True
...
def filter_products(df,p):
 ...
 if p.get('best'):out=out[out.badge.fillna('').str.contains('BEST',na=False)]
 if p.get('new'):out=out[out.badge.fillna('').str.contains('NEW',na=False)]
 return out
def humanize_filters(p):
 x=[]
 if p.get('brand'):x.append(p['brand'])
 if p.get('middle_category'):x.append(p['middle_category'])
 elif p.get('major_category'):x.append(p['major_category'])
 if p.get('min_max_size'):x.append(f"{int(p['min_max_size'])}까지")
 elif p.get('size_contains'):x.append(p['size_contains'])
 if p.get('concern'):x.append(p['concern'])
 if p.get('tpo'):x.append(p['tpo']+'룩' if p['tpo']=='출근' else p['tpo'])
 if p.get('color'):x.append(p['color'])
 if p.get('same_day'):x.append('당일발송')
 if p.get('max_price'):x.append(f"{int(p['max_price']/10000)}만원 이하")
 if p.get('best'):x.append('BEST')
 if p.get('new'):x.append('NEW')
 return x
